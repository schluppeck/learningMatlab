...
%% loop the loop over the whole thing...

[nX, nY, nZ, nT] = size(img);

%% loop the loop

results = nan(nX,nY,nZ, size(designMatrix,2)+1 );
r2 = nan(nX,nY,nZ);

t = tic;
h = waitbar(0,'Doing linear regression...');
for iZ = 1:nZ
    waitbar(iZ/nZ, h, ['doing slice ',num2str(iZ)])
    for iY = 1:nY
        for iX = 1:nX 
            y =  squeeze(img(iX,iY,iZ,:));
            b = X \ y;
            results(iX, iY, iZ, :) =  b;
            m =  X*b;
            r2(iX, iY, iZ) = 1- var(y-m)./var(y);
        end
    end
end
toc(t)
...
for iPlot = 1:4
    subplot(2,2,iPlot)
    imagesc(results(:,:,displaySlice,iPlot))
    axis image
    colormap(gray)
    colorbar
end
